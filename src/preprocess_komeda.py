...
# ディレクトリ名から座標ラベルへのマッピング（OriginalKomeda用）
LABEL_MAP = {
    'center': (756.0, 473.0),
    'topLeft': (20.0, 20.0),
    'topRight': (1492.0, 20.0),
    'bottomLeft': (20.0, 925.0),
    'bottomRight': (1492.0, 925.0)
}
...
def preprocess_komeda_images(raw_dir, output_dir, labels_csv_path, target_size=(224, 224)):
    """
    OriginalKomedaの画像を前処理して保存し、ラベルCSVを生成
    
    Args:
        raw_dir: 元画像のディレクトリ (OriginalKomeda/)
...
def main():
    parser = argparse.ArgumentParser(
        description='Preprocess OriginalKomeda images with face detection'
    )
    parser.add_argument(
        '--raw_dir',
        type=str,
        default='raw_data/OriginalKomeda',
        help='Directory containing raw images (with subdirectories)'
    )
    parser.add_argument(
        '--output_dir',
        type=str,
        default='data/images_komeda',
        help='Directory to save processed images'
    )
    parser.add_argument(
        '--labels_csv',
        type=str,
        default='data/labels_komeda.csv',
